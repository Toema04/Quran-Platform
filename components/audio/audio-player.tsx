...
import { Play, Pause, Music, Repeat, AlertCircle, X } from "lucide-react";
...
export function AudioPlayerUI() {
  ...
  if (!currentTrack && !audioError) return null;
  ...
  return (
    <div className="fixed bottom-16 md:bottom-0 left-0 right-0 z-40 bg-card/95 backdrop-blur-md border-t border-emerald-900/15 p-3 md:px-8 md:py-3 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-2 transition-all">
      {audioError ? (
        <div className="w-full flex items-center justify-between bg-red-500/10 border border-red-500/20 text-red-900 dark:text-red-200 px-4 py-2 rounded-2xl text-xs font-semibold">
          <div className="flex items-center gap-2">
            <AlertCircle className="h-4 w-4 text-red-600" />
            <span>{audioError}</span>
          </div>
          <button onClick={clearError} className="p-1 hover:bg-red-500/20 rounded-lg">
            <X className="h-4 w-4" />
          </button>
        </div>
      ) : currentTrack ? (
